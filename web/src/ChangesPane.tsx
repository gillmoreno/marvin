...
/** What git knows about the room's repo: files changed vs the branch base, and the diff of any of them. */
export function ChangesPane({ room, refreshKey, send }: { room: string; refreshKey: number; send: (m: ControlMessage) => void }) {
  ...
  const load = useCallback(async () => {
    try {
      const r = await fetch(`/api/changes?room=${encodeURIComponent(room)}`);
  ...
  useEffect(() => {
    if (!selected) return;
    let alive = true;
    fetch(`/api/changes/file?room=${encodeURIComponent(room)}&path=${encodeURIComponent(selected)}`)
      .then((r) => r.text())
  ...
  if (error) return <div className="changes"><p className="error">{error}</p></div>;
  if (!data) return <div className="changes"><p className="hint">reading git…</p></div>;
  if (!data.git) return <div className="changes"><p className="hint">{data.repo} is not a git repository yet. Ask {agent.name} to <code>git init</code> it, or create the room from a GitHub URL.</p></div>;
  ...
  return (
    <div className="changes">
      <header>
        ...
        <span className="btns">
          <button className="ghost" disabled={data.files.length === 0} onClick={() => send({ action: "ask", text: "commit the current changes with a clear message" })}>commit</button>
          <button disabled={!data.branch || data.branch === "main" || data.branch === "master"} onClick={() => send({ action: "ask", text: "push this branch and open a pull request" })}>open PR</button>
        </span>
      </header>
      {data.files.length === 0 ? (
