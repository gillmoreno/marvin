...
## Projects, model, machine notes

- **A room is a project**: one or more repos worked on together, each with a role (frontend, api, worker, lib,
  infra, docs) and optionally a branch. "New project" on the join screen picks them from your GitHub account
  (once you have connected it), from a folder on the machine, or from a pasted URL; the first repo is where the agent
  starts, all are readable and editable, and Changes gets one tab per repo. Settings → "This project" edits the list
  afterwards. Details: [`docs_and_changelog/projects.md`](docs_and_changelog/projects.md).
...
- The worker process is the machine. Repos live under `MARVIN_REPOS_DIR` (`/work/repos` in the image). The join
  screen lists projects and creates new ones from your GitHub repos, folders on the machine or git URLs; clones use
  the connected GitHub account of whoever clicks (else the machine's `GITHUB_TOKEN`). Projects created this way
  persist in `<state dir>/rooms.json`; `rooms.yaml` still declares static ones.
